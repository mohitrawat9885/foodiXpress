  ...
  const getNewOrders = async e => {
    try {
      ...
      if (res.status === 'success') {
        ...
        let indi = 0;
        for (let i in res.newOrders) {
          if (res.newOrders[i].status === 'Processing') {
            processingOrders[indi++] = res.newOrders[i];
          }
        }
        setNewOrders(processingOrders);
        setLoading(false);
      } else {
      ...
      <>
        <Header
          ...
          centerComponent={{
            ...
            style: {color: 'orange', fontSize: 25, justifyContent: 'center'},
          }}
          rightComponent={{
            icon: 'home',
            color: 'gray',
